...
private:
  ...
  std::size_t index;
  std::shared_ptr<std::vector<T>> lock_ptr() const {return wp_data.lock();}
  std::shared_ptr<std::vector<T>> check(std::size_t n, const std::string &msg) const;
};
...
template <typename T> inline
std::shared_ptr<std::vector<T>> BlobPtr<T>::check(std::size_t n, const std::string &msg) const
{
