...
{
    return ((((((id_ex_curr->icode) == (I_RMMOVQ) || (id_ex_curr->icode)
               == (I_PUSHQ)) & ((ex_mem_curr->icode) == (I_MRMOVQ) || 
              (ex_mem_curr->icode) == (I_POPQ))) & ((ex_mem_curr->destm)
             == (id_ex_curr->srca))) & ((ex_mem_curr->destm) != 
          (id_ex_curr->srcb))) ? (mem_wb_next->valm) : (id_ex_curr->vala));
}
